...
In a read-only planning mode, where the agent cannot write files or run mutating commands, use `npx --no-install gleip preflight --plan-mode "<task>"` and `npx --no-install gleip validate-plan --plan-mode --task "<task>" "<plan>"` instead. Both compute the same guidance and write nothing. Re-run them without `--plan-mode` once implementation is authorized. See [docs/plan-validation.md](docs/plan-validation.md#read-only-planning-mode).
...
| `npx gleip preflight --plan-mode "<task>"` | Print the brief and scope budget without writing anything.                   | Agents in a read-only planning mode; add `--json` for machine output.   |
...
| `npx --no-install gleip preflight --plan-mode "<task>"` | Compute and print the brief and scope budget in a read-only planning mode.           | None. Nothing is written. Add `--json` for machine-readable output.                                                                            |
| `npx --no-install gleip validate-plan --plan-mode --task "<task>" "<plan>"` | Validate a plan before a session exists, in a read-only planning mode. | None. Nothing is written. With an active session, `--task` is optional.                                                                        |
...
| `npx --no-install gleip check --plan-mode`            | Check current changes without writing the cache or recording evidence.                | None. Nothing is written.                                                                                                                      |
...
Gleip preserves the exact received task in `.gleip/canonical-task.json` and treats `.gleip/brief.md` as a derived navigation aid. The canonical task and active amendments remain authoritative for scope, plan validation, final reporting, generated agent instructions, and compression passthrough policy. The local requirement ledger tracks required, prohibited, optional, and informational obligations with deterministic source spans. A sentence is split into clauses when every clause reads as an instruction or a guardrail, so a multi-part instruction records each deliverable and its constraints separately rather than as one blurred obligation. Extraction remains deterministic and conservative: an instruction phrased without those signals is still recorded as a single requirement, so the canonical task stays authoritative over the derived ledger.
