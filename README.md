...
CI on high-confidence blocking findings. Scope expansion is warning-only in this
release because valid enterprise work often touches multiple files or modules.

Finding severities are:

- `info`: context only
- `warn`: advisory and non-blocking
- `fail`: strong local finding, non-blocking in 0.7.0 CI
- `blocking`: eligible to fail CI when its code is documented as blocking

Examples include `TEST_SKIPPED`, `TEST_DELETED`, `DEPENDENCY_FILE_CHANGED`,
`LOCKFILE_CHANGED`, `LOCAL_ARTIFACT_INCLUDED`, `NO_ACTIVE_SESSION`,
`SCOPE_EXPANSION_WARN`, `PLAN_REQUIRED_SECTION_MISSING`,
`PLAN_MENTIONED_FILE_MISSING`, `SCOPE_EXPANSION_RATIONALE_REQUIRED`,
`DEPENDENCY_REQUIREMENT_CONFLICT`, and `RISKY_CHANGE_RATIONALE_REQUIRED`.
Human output includes both code and severity, for example
`[TEST_SKIPPED] blocking: Skipped test added`.

`npx gleip check` remains advisory and keeps its existing local behavior.
`npx gleip check --ci` exits `1` only for `TEST_SKIPPED`, `TEST_DELETED`, or
`LOCAL_ARTIFACT_INCLUDED`; otherwise it exits `0`. `NO_ACTIVE_SESSION` exits non-zero
for commands that require an active session. Dependency, lockfile, plan structure,
and scope-expansion findings do not block CI in 0.7.0.
...
It checks named paths against the local repository and active scope budget, and it
detects explicit missing dependency requirements from local manifests. It does not
decide whether an implementation approach is correct, optimal, or semantically good.
Ordinary source/test expansion remains warning-based; hard-gated categories retain
stronger findings.

Scope budgets adapt to declared task breadth. A narrow `modify only` bugfix remains
tightly scoped, while an explicit release or feature task naming CLI, planner,
tests, docs, package metadata, and smoke tests allows those areas and raises the
expected file range. Files beyond the declared contract still require clarification
and a scope rationale. Package-version work does not implicitly permit dependency
additions, lockfile changes, CI, config, env, or secret changes.
...
Gleip 0.7.0 generates two local report artifacts:
